import numpy as np, torus_gf as e, time, sys
N=int(sys.argv[1]) if len(sys.argv)>1 else 4
t=e.Torus(N); nl=e.VarCell(t); P=e.cell_pairs(t)
...
print(f"torus N={N}  {t.nnodes} nodes  {6*N**3}+6 unknowns   VARIABLE CELL")
print(f"{'configuration':32s} {'want':8s} {'got':8s} {'base resid':>12s} {'sc resid':>11s} {'boxstrain':>10s}   s")
bad=0
for name,slots,want in CFG:
    t0=time.time(); r=nl.solve([P[s] for s in slots])
    got="legal" if r["legal"] else "illegal"
    if got!=want: bad+=1
    print(f"{name} {want:8s} {got:8s} {r['base']:12.3e} {r['sc']:11.3e} {r['strain']:10.2e} {time.time()-t0:5.1f}"
