...
for name,lst in (("PROVEN LEGAL (control)",LEG),("UNDECIDED",UND)):
    t0=time.time(); c=collections.Counter()
    for m in lst: c[legal(rods(m))['verdict']]+=1
    dt=time.time()-t0
    print(f"{name:26s} n={len(lst):3d}  {dict(c)}   {dt:.0f}s ({dt/len(lst):.2f}s each)",flush=True)
