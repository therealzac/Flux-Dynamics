...
;
  ...
  function drawXon(node, trail) {
    ...
    const moved = (node !== _xonNode && node !== null && node !== undefined);
    if (moved) {
      if (window._SIMTICK) window._SIMTICK();
      if (window._FLUXCLAMP) window._FLUXCLAMP(true);
    }
    if (moved) _tracer.push(P[node].clone());     // FROZEN AT RECORD TIME
    while (_tracer.length > _tracerMax) _tracer.shift();
    _xonNode = node;
    if (!_spark) {
      _spark = new THREE.Mesh(new THREE.SphereGeometry(0.13, 16, 12),
        new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: XON_A }));
      _spark.frustumCulled = false; scene.add(_spark);
      _wake = new THREE.Line(new THREE.BufferGeometry(),
        new THREE.LineBasicMaterial({ color: 0xffffff, transparent: true, opacity: WAKE_A }));
      _wake.frustumCulled = false; scene.add(_wake);
      applyXonDim();
    }
    _spark.position.copy(P[node]); _spark.visible = _xonDim > 0;
    // A TRACER, NOT A LOOKUP. The trail used to be a list of NODE IDS re-read
    // from P[] every frame, so as the lattice relaxed the whole history moved
    // with it -- the path the xon actually took was never on screen. Positions
    // are now captured the tick they happen and never touched again.
    const a = new Float32Array(Math.max(3, _tracer.length * 3));
    _tracer.forEach((p, k) => { a[k * 3] = p.x; a[k * 3 + 1] = p.y; a[k * 3 + 2] = p.z; });
    _wake.geometry.setAttribute('position', new THREE.BufferAttribute(a, 3));
    _wake.geometry.setDrawRange(0, _tracer.length);
    _wake.geometry.computeBoundingSphere();
    _wake.visible = _tracer.length > 1 && _xonDim > 0;
  }
  // Length is a layer slider, so the tracer can be a short wake or a full map
  // of everywhere the particle has been.
  window._TRAILLEN = (n) => { _tracerMax = Math.max(2, n | 0);
    while (_tracer.length > _tracerMax) _tracer.shift(); };
  ...
  window._XONOFF = () => { if (_spark) { _spark.visible = false; _wake.visible = false; }
    _xonNode = null; _tracer.length = 0; if (window._FLUXCLAMP) window._FLUXCLAMP(false); };
  ...
  (function stepperPanel() {
    ...
    box.id = 'nstep';
    // RETIRED. The stepper was scaffolding for debugging the nucleon CA move by
    // move; proton 87 replays a solved lap, so there is nothing to step through
    // and the card only competed for the right rail. The machinery stays --
    // window._NSTEP still drives it -- so unhide it if that debugging is needed
    // again. Same for the proton 87 optimiser card in lattice.html.
    box.hidden = true;
  ...
  if (window._registerLoop) {
    ...
    window._XZPROTON = (() => {
      ...
      // 120 stays banned.
      // ====================================================================
      // THE SOLVED LAP.  Generated by 080726/lap.js, which solves the formal
      // statement in 080726/PROBLEM.md exactly rather than by fitness:
      //
      //   requirement 1  every tet active for exactly 30 ticks, spread 0
      //   requirement 2  n90/(n90+n60) = 2/3 EXACTLY (160/80, no 0-turns),
      //                  and 2/3 at every scale -- deviation 19.2%% at w=3,
      //                  2.8%% at w=25, 0.7%% at w=100
      //
      // Both hold by construction, not by search: eight activations spaced L
      // apart give equal tet time, per-segment balance gives the ratio, and the
      // 60-spacing is carried in the DP state rather than hoped for.
      //
      // Vertices are the abstract compound -- oct at +-A e_i as 0..5 (poles are
      // 4 and 5), tet apexes at A(+-1,+-1,+-1) as 6..13 -- translated onto real
      // lattice nodes at spawn by translateOptWalk().
      // ====================================================================
      // COLOUR CHARGE.  Requirements 4 and 5 of 080726/PROBLEM.md.
      // ====================================================================
      // An actualized tet carries red, green or blue; each octahedron vertex
      // carries one of those or its anti (cyan, magenta, yellow). The pole the
      // tet touches takes the tet's colour, the opposite pole takes the anti,
      // and the two equator vertices ON THE TET'S OWN FACE take a colour/anti
      // pair between them -- they are ADJACENT on the equator, not antipodal,
      // so the pairing runs along the face edge and depends on which tet is
      // live. The remaining two equator vertices take the last pair.
      //
      // Colour is constant across an activation, so the whole colour problem is
      // one choice of 24 per activation. A tet is activated once per lap, so it
      // cannot see three colours evenly inside one lap: the colouring has a
      // period of THREE laps, 24 activations, after which every tet has been
      // each colour 8 times and every vertex each of six colours 4 times --
      // exactly, which is why window 24 below reads 0.00%.
      //
      //   window  3   tet 15.71%%   vertex 19.38%%
      //   window  6   tet 10.39%%   vertex 13.18%%
      //   window 12   tet 4.81%%   vertex 7.39%%
      //   window 24   tet 0.00%%   vertex 0.00%%      <- the full period, exact
      //
      // Best of 3000 exact colourings; every candidate is globally balanced, so
      // the search only moves the TIMING.
      const C_HEX = [0xff4d4d, 0x4dff70, 0x4d9cff,      // R G B
                     0x4dffff, 0xff4dff, 0xffff4d];     // C M Y (antis)
      const C_NAME = ['R','G','B','C','M','Y'];
      const P87_TETCOL = [1,2,0,1,2,0,1,0,0,1,2,0,1,2,0,1,2,0,1,2,0,1,2,2];
      // per activation, the colour index of abstract oct vertices 0..5
      const P87_VERTCOL = [
        [2,0,5,3,1,4],
        [0,4,3,1,2,5],
        [2,1,5,4,3,0],
      ...
      const P87_LAP = [
        6,4,0,5,1,4,2,5,3,1,2,0,4,1,5,2,4,3,5,0,4,1,2,0,
        3,1,5,0,4,1,12,4,3,5,2,4,1,5,0,4,2,5,3,0,2,1,3,4,
        2,5,1,4,0,2,1,3,4,2,5,3,13,5,3,4,2,5,3,1,2,0,5,1,
        4,3,5,2,1,3,0,4,1,5,2,4,3,0,2,1,3,0,9,5,3,4,2,1,
        3,0,5,1,4,2,5,3,0,2,1,5,0,4,2,5,3,4,2,1,3,0,2,1,
        11,5,2,4,3,5,0,4,1,3,0,2,1,5,0,4,1,5,3,4,2,1,3,0,
        4,1,5,2,4,3,8,4,0,5,1,4,3,5,2,0,3,1,4,0,5,1,3,0,
        2,5,3,4,0,5,1,2,0,3,1,2,10,4,1,5,0,4,3,5,2,4,1,5,
        0,3,1,2,4,3,5,1,4,0,2,1,3,0,4,1,5,0,7,5,2,4,3,5,
        2,0,3,1,2,5,3,4,0,5,1,2,0,3,4,2,5,1,4,0,3,1,2,0
      ];
      ...
      // ====================================================================
      // A FACE'S TICK COUNTS ONLY IF THE TET IS ACTUALLY THERE.
      // ====================================================================
      // "Ticks per face" is not "the xon is near this face" -- it is that an
      // actualized geometric tetrahedron EXISTS on that face in the solved
      // lattice. So it is measured, not assumed: all six pairwise distances
      // among the face's three vertices and its apex must be unit in P, the
      // solver's own answer. The vacuum can and does refuse a tet, and when it
      // does that face gets no tick, however long the xon loiters there.
      //
      // This is the only honest score. The optimiser's dwell is a PREDICTION;
      // this is the measurement, and the two are allowed to disagree.
      const UNIT_TOL = 1e-4;
      function tetActual(s) {
        const vs = [...s.face, s.apex];
        for (let i = 0; i < 4; i++) for (let j = i + 1; j < 4; j++)
          if (Math.abs(P[vs[i]].distanceTo(P[vs[j]]) - 1) > UNIT_TOL) return false;
        return true;
      }
      let faceTicks = null, actualTicks = 0;

      // Abstract oct vertex 0..5 -> real node. The abstract frame is
      // 0..5 = x+, x-, y+, y-, z+, z-, and build() lays the ring out as
      // (A,0,0) (0,A,0) (-A,0,0) (0,-A,0), so the ring order interleaves.
      const absOct = () => [ring[0], ring[2], ring[1], ring[3], pole[0], pole[1]];
      let actIdx = -1;
      // Repaint on ACTIVATION only -- colour is constant for the whole of a
      // tet's active window, which is what makes the balance a property of the
      // 24-activation period rather than of individual ticks.
      function applyColour() {
        actIdx = (actIdx + 1) % P87_TETCOL.length;
        const vc = P87_VERTCOL[actIdx], oct = absOct(), m = new Map();
        for (let k = 0; k < 6; k++) m.set(oct[k], C_HEX[vc[k]]);
        const hex = C_HEX[P87_TETCOL[actIdx]];
        // ONLY THE OCT CARRIES COLOURED VERTICES. The tet's colour belongs to
        // the SOLID, not to its apex node -- an apex is not an octahedron
        // vertex and has no colour charge of its own.
        window._NODECOL = m;
        // the solid itself, for the colour-charge layer to draw over the
        // white platonic faces
        window._CCTET = liveTet ? { nodes: [...liveTet.face, liveTet.apex], hex } : null;
      }
      // ====================================================================
      // LIVE VALIDATION.  Everything below is accumulated from what the engine
      // ACTUALLY DID -- turns as they were taken, colours as they were applied,
      // tets as they actualized -- not read back from the embedded tables. If
      // the tables and the run ever disagree, this is what notices.
      // ====================================================================
      let ledger = [], colViol = 0;
      const tetColSeen = Array.from({ length: 8 }, () => [0, 0, 0]);
      const vertColSeen = Array.from({ length: 6 }, () => new Array(6).fill(0));
      const LEDGER_MAX = 4000;
      function recordColour(faceIdx) {
        const vc = P87_VERTCOL[actIdx], tc = P87_TETCOL[actIdx];
        if (faceIdx >= 0) tetColSeen[faceIdx][tc]++;
        for (let v = 0; v < 6; v++) vertColSeen[v][vc[v]]++;
        // the two equator vertices on this tet's own face must be colour/anti,
        // its pole must carry the tet's colour, and the far pole the anti
        const s2 = liveTet; if (!s2) return;
        const oct = absOct(), ix = n => oct.indexOf(n);
        const pole = ix(s2.scWith), eq = s2.face.map(ix).filter(k => k !== pole && k >= 0);
        if (pole < 0 || eq.length !== 2) return;
        const anti = c => (c + 3) % 6;
        if (vc[pole] !== tc) colViol++;
        if (vc[pole === 4 ? 5 : 4] !== anti(tc)) colViol++;
        if (anti(vc[eq[0]]) !== vc[eq[1]]) colViol++;
      }
      // rms deviation of the 90-share from 2/3 over sliding windows
      function discrepancy() {
        const x = ledger, m = x.length; if (m < 8) return null;
        const pre = new Int32Array(2 * m + 1);
        for (let i = 0; i < 2 * m; i++) pre[i + 1] = pre[i] + x[i % m];
        const per = {}; let sum = 0, n = 0;
        for (const w of [3, 6, 12, 25, 50, 100]) { if (w > m) { per[w] = null; continue; }
          let se = 0; for (let i = 0; i < m; i++) { const d = (pre[i + w] - pre[i]) / w - 2 / 3; se += d * d; }
          per[w] = Math.sqrt(se / m); sum += per[w]; n++; }
        return { per, mean: n ? sum / n : null };
      }
      window._P87VALIDATE = () => {
        const n90 = ledger.filter(v => v === 1).length, n60 = ledger.length - n90;
        const f = faceReport();
        // BALANCE IS A PROPERTY OF THE PERIOD, NOT OF EVERY MOMENT. Colour
        // repeats every 24 activations; stop partway and the counts are
        // legitimately uneven by however many activations into the period we
        // are. Demanding evenness at all times reported a failure that was
        // just "mid-cycle". The bound below is exact: after n activations the
        // spread cannot exceed what the incomplete tail can contribute.
        const nAct = stats ? stats.tets : 0, part = nAct % P87_TETCOL.length;
        const spreadOf = r => Math.max(...r) - Math.min(...r);
        const vSpread = Math.max(...vertColSeen.map(spreadOf));
        const tSpread = Math.max(...tetColSeen.map(spreadOf));
        const tOK = tSpread <= Math.ceil(part / 8);
        const vOK = vSpread <= part;
        // Face time is even PER LAP. Stopping mid-lap, the faces already
        // visited this lap are ahead of the rest by up to one segment -- so the
        // honest bound is how far into the lap we are, not zero.
        const L = walk ? walk.length / 8 : 0;
        const inLap = walk ? (stats ? stats.steps % walk.length : 0) : 0;
        const faceBound = Math.min(L, inLap);
        const faceOK = !f || f.spread <= faceBound;
        return { ledger: ledger.length, n90, n60, n0: stats ? stats.n0 : 0,
          faceBound, faceOK, inLap, segLen: L,
          ratio: (n90 + n60) ? n90 / (n90 + n60) : null,
          disc: discrepancy(), faces: f,
          tetCol: tetColSeen.map(r => r.slice()), vertCol: vertColSeen.map(r => r.slice()),
          tetColOK: tOK, vertColOK: vOK, colViol,
          tSpread, vSpread, nAct, part, periodDone: part === 0 && nAct > 0,
          tets: stats ? stats.tets : 0, refused: stats ? stats.refused : 0,
          steps: stats ? stats.steps : 0, lap: walk ? walk.length : 0 };
      };
      const colourState = () => actIdx < 0 ? null : {
        n: actIdx, of: P87_TETCOL.length,
        tet: C_NAME[P87_TETCOL[actIdx]],
        verts: P87_VERTCOL[actIdx].map(c => C_NAME[c]).join('') };
      function sampleActual() {
        if (!faceTicks) faceTicks = new Map(stella.map(s => [s.apex, 0]));
        let any = false;
        for (const s of stella) if (tetActual(s)) { faceTicks.set(s.apex, faceTicks.get(s.apex) + 1); any = true; }
        if (any) actualTicks++;
      }
      const faceReport = () => {
        if (!faceTicks) return null;
        const v = [...faceTicks.values()], mn = Math.min(...v), mx = Math.max(...v);
        const mean = v.reduce((a, b) => a + b, 0) / v.length;
        const sd = Math.sqrt(v.reduce((a, b) => a + (b - mean) ** 2, 0) / v.length);
        return { ticks: v, min: mn, max: mx, spread: mx - mn, sd: +sd.toFixed(3),
                 ticksWithATet: actualTicks };
      };

      // The optimiser works on the ideal compound (oct at +-A e_i, apexes at
      // A(+-1,+-1,+-1)); the engine works on lattice node ids. Same ideal
      // coordinates on both sides, so the translation is exact -- match on the
      // rounded ideal position and refuse the walk outright if any vertex fails
      // to land, rather than running a walk that is quietly wrong.
      function translateOptWalk(absV) {
        const AA = 1 / Math.SQRT2, key = p => p.map(c => (c / AA).toFixed(3)).join(',');
        const byPos = new Map();
        for (const [nid, p] of ideal) byPos.set(key(p), nid);
        const absPos = [];
        for (let a = 0; a < 3; a++) for (const s of [1, -1]) { const p = [0, 0, 0]; p[a] = s * AA; absPos.push(p); }
        for (const x of [1, -1]) for (const y of [1, -1]) for (const z of [1, -1]) absPos.push([x * AA, y * AA, z * AA]);
        const out = [];
        for (const i of absV) { const nid = byPos.get(key(absPos[i])); if (nid === undefined) return null; out.push(nid); }
        return out;
      }
      ...
      async function start() {
        ...
        // A LAP FROM THE OPTIMISER WINS. It is exact where the samplers are
        // approximate: composition, face time and closure are held by
        // construction rather than scored. Translated and re-verified here --
        // if any turn on THIS lattice is illegal the lap is dropped, so a bad
        // hand-off degrades to the local generator instead of running wrong.
        const absWalk = (Array.isArray(window._P87WALK) && window._P87WALK.length)
                        ? window._P87WALK : P87_LAP;
        if (absWalk && absWalk.length) {
          const V = translateOptWalk(absWalk);
          if (V) { const nn = V.length, T = [];
            for (let i = 0; i < nn; i++) T.push(turn(V[(i - 1 + nn) % nn], V[i], V[(i + 1) % nn]));
            if (!T.some(x => x === null)) {
              walk = V; turns = T; at = 0; liveTet = null;
              faceTicks = null; actualTicks = 0; actIdx = -1;
              window._NODECOL = null; window._CCTET = null;
              ledger.length = 0; colViol = 0;
              tetColSeen.forEach(r => r.fill(0)); vertColSeen.forEach(r => r.fill(0));
              window._EXACTSEG = (nn / 8) - 1;
              stats = { steps: 0, tets: 0, refused: 0, n90: 0, n60: 0, n0: 0, src: 'optimiser' };
              active.clear();
              for (const [k, pr] of ringKeys) active.set(k, pr);
      ...
      async function step() {
        ...
        stats.steps++;
        if (t === 90) { stats.n90++; ledger.push(1); }
        else if (t === 60) { stats.n60++; ledger.push(0); }
        else if (t === 0) stats.n0 = (stats.n0 || 0) + 1;
        if (ledger.length > LEDGER_MAX) ledger.shift();
        drawXon(v, walk.slice(Math.max(0, at - 11), at + 1));
        // ACTIVATION IS AN EDGE, NOT A VERTEX. A tet is actualized when its
        // apex-to-pole shortcut is TRAVERSED -- either direction -- and stays
        // active until a different one is. Firing on arrival at an apex was
        // wrong: a walk can pass through an apex on its two base edges without
        // activating anything, and on the solved lap that difference is what
        // makes tet time even instead of ragged.
        const prevNode = walk[(at - 1 + walk.length) % walk.length];
        const want = stella.find(s2 => (s2.apex === prevNode && s2.scWith === v)
                                    || (s2.apex === v && s2.scWith === prevNode)) || null;
        ...
        if (want && (!liveTet || liveTet.apex !== want.apex)) {
          ...
          if (ok) {
            ...
            applyColour();
            recordColour(stella.findIndex(z => z.apex === want.apex));
            lastChange = 'tet'; restate(true); await settle(); detect(); }
        }
        // AFTER the lattice has settled, so what is counted is what exists.
        sampleActual();
        return state();
      }
      ...
      const state = () => ({ step: at, of: walk ? walk.length : 0,
        ...
        faces: faceReport(), colour: colourState(),
        ...stats });
    ...
    // ====================================================================
    // BALANCED 9 -- the exact solution to the identity requirement.
    // ====================================================================
    // A closed nine-step walk whose turn sequence is 90 60 90 90 60 90 90 60 90:
    // ratio exactly 2/3, and EVERY window of three holds exactly two 90s. That
    // is the balanced (Christoffel) word, the 2/3 target met at every scale at
    // once, and it is the floor -- nothing does better.
    //
    // It does NOT fit inside one stella octangula. It runs across neighbouring
    // cells, which is the whole reason it can be perfect: confined to a single
    // compound, every apex visit forces a 60 (an apex's three neighbours are
    // mutually unit-distance) and perfect balance is unreachable.
    //
    // Found on the tet-oct honeycomb -- the actualized void lattice with one
    // flux mode live, where every node has 8 base + 4 shortcut neighbours = the
    // 12 of FCC -- then mapped back onto real lattice nodes through the oct.
    window._BAL9 = (() => {
      // the walk in honeycomb coordinates, relative to its own oct centre
      const FCC = [[0,0,0],[1,1,0],[2,0,0],[1,0,-1],[2,0,-2],
                   [1,0,-3],[1,-1,-2],[1,0,-1],[1,-1,0]];
      let walk = null, at = 0, scKeys = [], stats = null;

      function build() {
        _sync();
        // NOT K(). The file's K(i,j) is i*100000+j and is NOT symmetric, so a
        // base-edge lookup with the endpoints the other way round silently
        // misses -- which is exactly how this failed the first time.
        const KS = (i, j) => Math.min(i, j) * 100000 + Math.max(i, j);
        const bs = new Set(); for (const [i, j] of BASE) bs.add(KS(i, j));
        const AXx = 0, AZ = 4;
        let best = null;
        for (let i = 0; i < NODE.length; i++) {
          const a = SCOPT.get(i + ':' + AXx), d = SCOPT.get(i + ':' + AZ);
        ...
        if (!best) return false;
        const ring = best.r, pole = [];
        for (let v = 0; v < NODE.length; v++) { if (ring.includes(v)) continue;
          if (ring.every(r => bs.has(KS(v, r)))) pole.push(v); }
        if (pole.length !== 2) return false;
        // oct centre, and the honeycomb basis read off the ring and one pole
        const C = [0,1,2].map(k => (ring.reduce((s,v)=>s+NODE[v][k],0)
                                    + pole.reduce((s,v)=>s+NODE[v][k],0)) / 6);
        const sub = (a,b) => [a[0]-b[0], a[1]-b[1], a[2]-b[2]];
        const M = [sub(NODE[ring[0]], C), sub(NODE[ring[1]], C), sub(NODE[pole[0]], C)];
        const out = [];
        for (const f of FCC) {
          const g = [f[0]-1, f[1], f[2]];
          const e = [0,1,2].map(k => Math.round(C[k] + g[0]*M[0][k] + g[1]*M[1][k] + g[2]*M[2][k]));
          const n = KEY.get(e.join(',')); if (n === undefined) return false;
          out.push(n);
        }
        // every consecutive pair must be a real edge; collect the shortcuts
        scKeys = [];
        for (let i = 0; i < out.length; i++) {
          const a = out[i], b = out[(i+1) % out.length];
          if (bs.has(KS(a, b))) continue;
          const nk = scKeyOf(a, b); if (!nk) return false;
          scKeys.push(nk);
        }
        walk = out; return true;
      }

      async function start() {
        if (!build()) return { ok:false, why:'no XZ octahedron here' };
        active.clear();
        for (const nk of scKeys) active.set(nk[0], nk[1]);
        lastChange = 'tet'; restate(true); await settle(); detect();
        at = 0; stats = { steps:0, n90:0, n60:0, n0:0, refused:0 };
        drawXon(walk[0], [walk[0]]);
        return state();
      }
      // Turn measured from the SOLVED positions, not asserted: the chord
      // between the two neighbours, in edge units.
      const measured = i => {
        const n = walk.length, u = walk[(i-1+n)%n], v = walk[i], w = walk[(i+1)%n];
        const L = P[u].distanceTo(P[w]);
        return Math.abs(L-1) < 2e-3 ? 60 : Math.abs(L-Math.SQRT2) < 2e-3 ? 90
             : Math.abs(L-2) < 2e-3 ? 0 : null; };
      async function step() {
        if (!walk) return { ok:false, why:'call start() first' };
        at = (at + 1) % walk.length;
        const t = measured(at);
        ...
        else if (t === 0) stats.n0++; else stats.refused++;
        drawXon(walk[at], walk.slice(Math.max(0, at-8), at+1));
        return state();
      }
      const state = () => ({ step:at, of: walk ? walk.length : 0,
        node: walk ? walk[at] : null, turns: walk ? walk.map((_,i)=>measured(i)) : null,
        ratio90: stats && (stats.n90+stats.n60) ? +(stats.n90/(stats.n90+stats.n60)).toFixed(4) : null,
        rods: active.size, ...stats });
      return { start, step, state, walk: () => walk && walk.slice() };
    })();

    window._registerLoop('balanced-9', 'balanced 9',
      'The exact solution to nucleon identity: a closed nine-step walk whose '
      + 'turn sequence is 90 60 90 90 60 90 90 60 90. Ratio exactly two thirds, '
      + 'and every window of three holds exactly two 90s -- the 2/3 condition '
      + 'met at EVERY scale simultaneously, which is the floor. It does not fit '
      + 'inside one stella octangula: confined to a single compound every apex '
      + 'visit forces a 60, and perfect balance is impossible. Three of its nine '
      + 'edges are shortcuts, six are base.',
      async (tok) => {
        while (window._loopAlive(tok)) {
          const s0 = await window._BAL9.start();
          if (!s0 || s0.ok === false) { await breathe(); continue; }
          while (window._loopAlive(tok)) {
            await window._BAL9.step();
